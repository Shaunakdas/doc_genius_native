...
import { SERVER, URL,
  // BASE_URL, STUDENT_ROLE, COUNSELOR_ROLE, SENDBIRD_APP_ID,
  // ADMIN_BASE_URL, IMAGE_SERVICE_URL 
} from './constants';
...
export const gamesAPI = async (authToken, page = 1, limit = 10) => {
  ...
  const url = `${URL(SERVER)}/homepage?${queryString}`;
...
export const standardsAPI = async () => {
  const url = `${URL(SERVER)}/standards`;
